  ...
  class Distribution
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Distribution` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Distribution`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @bucket_by.nil?
        invalid_properties.push('invalid value for "bucket_by", bucket_by cannot be nil.')
      end
    ...
    def bucket_by=(bucket_by)
      if bucket_by.nil?
        fail ArgumentError, 'bucket_by cannot be nil'
      end
